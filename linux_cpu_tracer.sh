...
cat /var/log/audit/audit.* | grep type=SYSCALL | awk -F ' ' '{print $4, $26}' | sort | uniq -c | sort -k1rn | head -n 10 >> $DIRNAME/auditd_initiators.txt
echo "" >> $DIRNAME/auditd_initiators.txt
EOF
...
sudo service auditd status > $DIRNAME/service_auditd_status.txt
sudo service mdatp status > $DIRNAME/service_mdatp_status.txt

echo -e ""
echo -e " *** Finished collecting data."
...
case $1 in
		...
		-h) 
			...
			echo "	      One sample every 30 minutes for 24 hours: './linux_cpu_tracer.sh -l 48 1800'  "
			echo "	      One sample every 15 minutes for 24 hours: './linux_cpu_tracer.sh -l 96 900'   "
			echo "	      One sample every 10 minutes for 24 hours: './linux_cpu_tracer.sh -l 144 600'  "
			echo "	      One sample every 5 minutes for 24 hours:  './linux_cpu_tracer.sh -l 288 300'  "
			echo "	      One sample every 2 minutes for 24 hours:  './linux_cpu_tracer.sh -l 1440 60'  "
			echo "     Note:    "
