...
echo "set terminal wxt size 1800,600"  >> $DIRNAME/plot.plt 
...
echo "set key noenhanced" >> $DIRNAME/plot.plt
echo "set key right top outside" >> $DIRNAME/plot.plt
echo "plot 'graphs/1_$PID1_NAME.plt' with linespoints title '$PID1_NAME', 'graphs/2_$PID2_NAME.plt' with linespoints title '$PID2_NAME', 'graphs/3_$PID3_NAME.plt' with linespoints title '$PID3_NAME', 'graphs/4_$PID4_NAME.plt' with linespoints title '$PID4_NAME'" >> $DIRNAME/plot.plt
}
...
case $1 in
		...
		-h) 
			echo " *** Usage: ./linux_cpu_tracer_v2.sh -s <time to capture in seconds>"
		;;
