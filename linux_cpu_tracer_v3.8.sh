...
case $1 in
		...
		-h) 
			...
			echo "     Usage:./linux_cpu_tracer.sh -ps <time to capture in seconds>, performance short-mode."
		    echo "	   ./linux_cpu_tracer.sh -pl <nr. of samples> <sampling interval in seconds>, performance long-mode." 
			echo "                   Can  be used with 'nohup' and sent to background [&] in long run captures, when remote "
			echo "                   sessions need to be disconnected. Example: 'nohup ./linux_cpu_tracer.sh -pl 120 0.6 &'"
			...
			echo "           ./linux_cpu_tracer.sh -ct, runs 'mdatp connectivity test'. Can take long if there are"
